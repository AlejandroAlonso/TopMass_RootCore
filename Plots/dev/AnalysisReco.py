...
##
##
##	DEPRECATED, go for Multithreated versions!! NOT THISONE
##
##
#####
...
class analysisReconstruction:
	...
	def DoAnalysis(self,fortchain,keys,norm,requested_bjets,data=0):
	 Histos         = {}
	 MomentsAll  = {}
	 for i in keys:
		Histos [i] = {} 
		Histos [i] ["ee_2016"] 			= {}
		Histos [i] ["ee_2016"] ["OS"]  		= self.ToStoreHistos() 
		Histos [i] ["ee_2016"] ["SS"]  		= self.ToStoreHistos() 
		Histos [i] ["emu_2016"] 		= {} 
		Histos [i] ["emu_2016"] ["OS"]  	= self.ToStoreHistos() 
		Histos [i] ["emu_2016"] ["SS"]  	= self.ToStoreHistos() 
		Histos [i] ["mumu_2016"] 		= {} 
		Histos [i] ["mumu_2016"] ["OS"]  	= self.ToStoreHistos() 
		Histos [i] ["mumu_2016"] ["SS"]  	= self.ToStoreHistos() 
		MomentsAll[i] = {}
		MomentsAll[i] ["ee_2016"] 	= Moments() 
		MomentsAll[i] ["emu_2016"] 	= Moments() 
		MomentsAll[i] ["mumu_2016"] 	= Moments() 
		entries =  fortchain[i].GetEntriesFast()
		print entries 
		fortchain[i].SetCacheSize(1000000000000000000)
		...
		for event in fortchain[i]: 
			...
			if not data and event.randomRunNumber < 297730:
...
#				continue
			...
			for k in range (len(event.jet_pt)):	
				...
				Histos [i] [selection] [sign] ["jet_p_T"] 		.Fill( event.jet_pt[k]/1000.          , weight)
				Histos [i] [selection] [sign] ["jet_eta"] 		.Fill( event.jet_eta[k]        		 , weight)
				Histos [i] [selection] [sign] ["jet_jvf"] 		.Fill( event.jet_jvt[k]        		 , weight)
				Histos [i] [selection] [sign] ["jet_b-jet-score"] 	.Fill( event.jet_mv2c20[k]   	 , weight)
				jets.append(jet)
				if event.jet_mv2c20[k] > -0.0436:
					Histos [i] [selection] [sign] ["bjet_p_T"] 		.Fill( event.jet_pt[k]/1000.          , weight)
					Histos [i] [selection] [sign] ["bjet_eta"] 		.Fill( event.jet_eta[k]        		 , weight)
					Histos [i] [selection] [sign] ["bjet_jvf"] 		.Fill( event.jet_jvt[k]        		 , weight)
					Histos [i] [selection] [sign] ["bjet_b-jet-score"] 	.Fill( event.jet_mv2c20[k]   	 , weight)
					bjets.append(jet)
			...
			if len(jets) > 0:
				Histos [i] [selection] [sign] ["LeadingJet_p_T"] 		.Fill( jets[0].tlv().Pt()	, weight)
				Histos [i] [selection] [sign] ["LeadingJet_eta"]		.Fill( jets[0].tlv().Eta()	, weight)
				Histos [i] [selection] [sign] ["LeadingJet_jvf"] 		.Fill( jets[0].tlv().Phi()	, weight)
				Histos [i] [selection] [sign] ["LeadingJet_b-score"] 		.Fill( jets[0].bscore()		, weight)

			if len(bjets) > 0:
				Histos [i] [selection] [sign] ["LeadingbJet_p_T"] 		.Fill( bjets[0].tlv().Pt()	, weight)
				Histos [i] [selection] [sign] ["LeadingbJet_eta"]		.Fill( bjets[0].tlv().Eta()	, weight)
				Histos [i] [selection] [sign] ["LeadingbJet_jvf"] 		.Fill( bjets[0].tlv().Phi()	, weight)
				Histos [i] [selection] [sign] ["LeadingbJet_b-score"] 		.Fill( bjets[0].bscore()		, weight)

			if (not data):
				Histos [i] [selection] [sign] ["weight"]		.Fill( event.weight_mc *event.weight_pileup * event.weight_leptonSF * event.weight_bTagSF_77 * event.weight_jvt	)	
			Histos [i] [selection] [sign] ["mu"] 			.Fill( event.mu                    	, weight)		
			Histos [i] [selection] [sign] ["Njets"] 		.Fill( len(event.jet_pt)    		, weight)
			Histos [i] [selection] [sign] ["Nb-jets"] 			.Fill( nbjets        , weight)

			Histos [i] [selection] [sign] ["met_phi"] 		.Fill( event.met_phi			, weight)
			Histos [i] [selection] [sign] ["met_met"] 		.Fill( event.met_met/1000.		, weight)	

			for k in range(len(event.el_pt)):
				Histos [i] [selection] [sign] ["el_p_T"] 	.Fill( event.el_pt[k]/1000.          	, weight)
				Histos [i] [selection] [sign] ["el_eta"] 	.Fill( event.el_eta[k]          	, weight)
				...
				if ( event.el_charge[k] > 0):
					Histos [i] [selection] [sign]["PositiveLepton_p_T"]	.Fill( event.el_pt[k]/1000.             , weight)
					Histos [i] [selection] [sign]["PositiveLepton_eta"]	.Fill( event.el_eta[k]                  , weight)
				elif ( event.el_charge[k] < 0):
					Histos [i] [selection] [sign]["NegativeLepton_p_T"]	.Fill( event.el_pt[k]/1000.             , weight)
					Histos [i] [selection] [sign]["NegativeLepton_eta"]	.Fill( event.el_eta[k]                  , weight)

			for k in range(len(event.mu_pt)):
				Histos [i] [selection] [sign] ["mu_p_T"] 	.Fill( event.mu_pt[k]/1000.           	, weight)
				Histos [i] [selection] [sign] ["mu_eta"] 	.Fill( event.mu_eta[k]          	, weight)
				lepton = particle( event.mu_pt[k]/1000., event.mu_eta[k], event.mu_phi[k], 0.105, 13, event.mu_charge[k] ) 
				leptons.append(lepton)
				if ( event.mu_charge[k] > 0):
					Histos [i] [selection] [sign]["PositiveLepton_p_T"]	.Fill( event.mu_pt[k]/1000.             , weight)
					Histos [i] [selection] [sign]["PositiveLepton_eta"]	.Fill( event.mu_eta[k]                  , weight)
				elif ( event.mu_charge[k] < 0):
					Histos [i] [selection] [sign]["NegativeLepton_p_T"]	.Fill( event.mu_pt[k]/1000.             , weight)
					Histos [i] [selection] [sign]["NegativeLepton_eta"]	.Fill( event.mu_eta[k]                  , weight)
			...
			Histos [i] [selection] [sign] ["LeadingLepton_p_T"] 	.Fill( leptons[0].tlv().Pt(), 	weight)
			Histos [i] [selection] [sign] ["LeadingLepton_eta"] 	.Fill( leptons[0].tlv().Eta(), 	weight)
			Histos [i] [selection] [sign] ["SubLeadingLepton_p_T"] 	.Fill( leptons[1].tlv().Pt(), 	weight)
			Histos [i] [selection] [sign] ["SubLeadingLepton_eta"] 	.Fill( leptons[1].tlv().Eta(), 	weight)

			Histos [i] [selection] [sign] ["delta_phi"] 		.Fill( leptons[0].tlv().Pt() - leptons[1].tlv().Phi() , weight)
			Histos [i] [selection] [sign] ["M_ll"] 			.Fill( (leptons[0].tlv() + leptons[1].tlv()).M(), weight)
			...
			Histos [i] [selection] [sign] ["Hist_pT(l+)"] 			.Fill(mom_1, weight)	
			Histos [i] [selection] [sign] ["Hist_pT(l+l-)"] 		.Fill(mom_2, weight)
			Histos [i] [selection] [sign] ["Hist_M(l+l-)"] 			.Fill(mom_3, weight)
			Histos [i] [selection] [sign] ["Hist_E(l+)+E(l-)"] 		.Fill(mom_4, weight)
			Histos [i] [selection] [sign] ["Hist_pT(l+)+pT(l-)"] 		.Fill(mom_5, weight	)
			if (sign == "OS"):
					MomentsAll[i][selection].fill( "pT(l+)" 	 	, mom_1, weight)
					MomentsAll[i][selection].fill( "pT(l+l-)" 	 	, mom_2, weight)
					MomentsAll[i][selection].fill( "M(l+l-)" 	 	, mom_3, weight)
					MomentsAll[i][selection].fill( "E(l+)+E(l-)"  	, mom_4, weight)
					MomentsAll[i][selection].fill( "pT(l+)+pT(l-)"	, mom_5, weight)
		print "Sample Over"
